...
|---|---|---|
| OQ-VPS-SETUP-01 | What is the selected VPS provider and plan for the initial Phase 1 VPS? | Resolved 2026-05-01: Hostinger KVM 8 (8 vCPU shared / 32 GB RAM / 400 GB NVMe), 12-month term, US region, Ubuntu 24.04 LTS. Rationale: matches Phase 1 sizing with comfortable headroom across the documented multi-lane architecture in `vps_development_environment.md`; aligns with locked SDR Decision 18 (Hostinger named) and EW v1.1 deployment target; 12-month term avoids 24-month commitment and provides clean re-evaluation point before renewal pricing applies. Future provider/plan changes deferred to a separate strategy decision. |
