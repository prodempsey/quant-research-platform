...
This document (docs/current_claude_context_handoff.md) — the map.
docs/strategy_decision_record.md — the canonical SDR. v1.0 LOCKED. 18 numbered decisions.
docs/engineering_workflow.md — the canonical EW. v1.5 LOCKED. Defines the Approver/Builder/Reviewer roles, the section-drafting workflow, the Approval Matrix (§2.3), the spec-section template (§3.2), the assumption classification rules (§3.3), the QA review checklist (§9), and the configuration-management discipline (§7).
docs/traceability_matrix.md — the canonical traceability matrix. Currently v0.4 (Sections 1, 2, 3a v1.0 LOCKED merged; Sections 3b, 3c, 4, 5, 6 still pending).
docs/engineering_spec/ — directory of locked Engineering Specification sections. Currently:
...
docs/reviews/ — directory of approval notes, traceability companion artifacts, and Section-level handoff packets. Read whichever ones are relevant to the task at hand. Notable files:
...
Locked artifacts
ArtifactStatusPathStrategy Decision Recordv1.0 LOCKEDdocs/strategy_decision_record.mdEngineering Workflowv1.5 LOCKEDdocs/engineering_workflow.mdEngineering Spec — Section 1 (Architecture Overview)v1.0 LOCKEDdocs/engineering_spec/01_architecture_overview.mdEngineering Spec — Section 2 (Data Layer)v1.0 LOCKEDdocs/engineering_spec/02_data_layer.mdEngineering Spec — Section 3a (Feature Engineering)v1.0 LOCKED / APPROVEDdocs/engineering_spec/03a_feature_engineering.mdTraceability Matrixv0.4docs/traceability_matrix.md
...
Read this handoff, then read the SDR, EW, all locked sections (1, 2, 3a), and the section-specific handoff packet under docs/reviews/.
Confirm receipt of the handoff packet and acknowledge any Open Questions in it before producing v0.1.
Use the EW §3.2 eleven-field template in order. Classify every assumption per EW §3.3.
Output the draft as a markdown artifact with the exact canonical filename (e.g., 03b_target_generation.md).

"Apply QA revisions to v0.X"

Read the QA revision list carefully. Each revision typically names affected sections.
Apply each revision via targeted str_replace edits, one at a time. Do not rewrite the whole document.
After each revision, verify with grep or by viewing the affected region.
Add a new changelog entry summarizing the revisions; preserve historical changelog entries verbatim.
Output the revised draft (e.g., promoted from v0.X to v0.X+1).

"Promote v0.X to v1.0 LOCKED"

Produce three artifacts in the established shape:
...
Do not apply the traceability companion's rows to docs/traceability_matrix.md unless explicitly instructed.

"Apply Section 3X traceability matrix updates"

Read the Section 3X traceability companion under docs/reviews/.
Read the current docs/traceability_matrix.md.
Apply replacement rows surgically. Preserve verbatim the rows for decisions not affected.
Add a version-history entry to the matrix's Notes section.
Update the document status header and the in-spec interpretation note.
